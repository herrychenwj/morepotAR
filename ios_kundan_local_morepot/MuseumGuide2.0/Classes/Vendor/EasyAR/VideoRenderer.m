...
- (void)render:(easyar_Matrix44F *)projectionMatrix cameraview:(easyar_Matrix44F *)cameraview size:(easyar_Vec2F *)size scale:(VideoScale)scale{

    float size0 = [[size.data objectAtIndex:0] floatValue]/scale.scalex;
    float size1 = [[size.data objectAtIndex:1] floatValue]/scale.scaley;
    
    glBindBuffer(GL_ARRAY_BUFFER, vbo_coord_box);
    
    const GLfloat cube_vertices[4][3] = {{size0, size1+ scale.x, 0.f},{size0, -size1+ scale.y, 0.f},{-size0, -size1+ scale.z , 0.f},{-size0, size1+ scale.z, 0.f}};
    glBufferData(GL_ARRAY_BUFFER, sizeof(cube_vertices), cube_vertices, GL_DYNAMIC_DRAW);
