...
+ (NSString *)mj_localizedStringForKey:(NSString *)key value:(NSString *)value
{
    ...
    NSString *cons = [[NSUserDefaults standardUserDefaults]objectForKey:@"localizable"];
    NSString *sufxx = [[bundle.bundlePath lastPathComponent] stringByDeletingPathExtension];
    if (bundle == nil || ![sufxx containsString:cons]) {
        // （iOS获取的语言字符串比较不稳定）目前框架只处理en、zh-Hans、zh-Hant三种情况，其他按照系统默认处理
        NSString *language;
        NSString *cons = [[NSUserDefaults standardUserDefaults]objectForKey:@"localizable"];
        if ([cons hasSuffix:@"en"]) {
            language = @"en";
        }else{
